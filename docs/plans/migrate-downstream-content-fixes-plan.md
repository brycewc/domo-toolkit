...
1. **No inter-beast-mode id remap.** Each beast mode is column-rewritten ([rewriteBeastModeColumns](src/services/columnRewriter.js#L74-L78)) and has its origin _dataset_ id swept to the target ([buildBeastModeEntry](src/services/migrateDownstreamContent.js#L988-L995)). Nothing rewrites references to **other beast modes**. A nested beast mode's definition still embeds the _origin_ `calculation_<uuid>` of the beast mode it references, which does not exist on the target.
...
**Fusions and template/SQL views are different objects, not variants — treat them as fully separate paths with no shared fetch/rewrite/PUT.** This mirrors precedent already in the codebase ([getDatasetsForView](src/services/datasets.js#L288) branches on `schema.views` vs `schema.select.selectBody`; [isViewType](src/services/datasets.js#L527-L538) distinguishes `dataset-view` from `datafusion`). The migration swap is currently the one place that wrongly collapses them; re-separate it:

1. **Route by type at the entry point, not as a branch deep inside the PUT.** Determine fusion-ness during downstream-dataset discovery — [getDownstreamLineage](src/services/migrateDownstreamContent.js#L126-L206) already bulk-fetches dataset metadata (`part=core`); have it carry the `dataProviderType`/`type` so each dataset item knows if it is a `datafusion` (reuse the discriminator behind [isViewType](src/services/datasets.js#L527-L538)). Then [dispatchSwap](src/services/migrateDownstreamContent.js#L1047-L1088) routes fusions to a new **`swapFusionInput`** and template/SQL views to the existing **`swapDatasetViewInput`** — two sibling functions, just like cards / dataflows / views are already separate cases in `dispatchSwap`. The fusion path never touches `putDatasetViewInPage`, and the template-view path is left exactly as-is. Fall back to `isFusionView(definition)` on the fetched def only if a metadata signal is unavailable.

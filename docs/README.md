...
- **Copy Color Rules** - Copy a dataset's color rules (conditional formats) to another dataset. Pick exactly which rules to copy with per-rule checkboxes, grouped by the column each one tests and shown with its fill color, and choose whether to replace the destination's existing rules or add to them. Column references are validated against the destination's schema; Beast Mode references are name-matched between source and destination so rules keep working across datasets with equivalent calculations.

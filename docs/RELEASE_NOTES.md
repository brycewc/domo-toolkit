...
- New "Cancel Update" action button on datasets — appears whenever the dataset's stream has an execution in `ACTIVE` state
- Calls `PUT /api/data/v1/streams/{streamId}/executions/{executionId}/abort` to free the stream so subsequent scheduled runs can proceed
- Fills a gap where Domo's UI shows the "Storing…" indicator indefinitely with no recovery affordance — previously a stuck connector run blocked every subsequent scheduled update with "there is already an update storing", forcing a support ticket
...
- DataList header title now exposes the full text via a native `title` hover-tooltip when truncated by `line-clamp-1` (matches the same hover-overflow behavior already in place for the subtext row). Only applied when the title prop is a string — non-string ReactNode titles are skipped since the HTML attribute can't render them
...
- Cleaned up `src/components/icons/`: dropped the `-icon` suffix from the 14 SVGs that had it. Seven were renamed (e.g. `align-center-icon.svg` → `align-center.svg`, producing `IconAlignCenter`); the other seven collided with an existing non-suffixed file — four were byte-identical duplicates and three were visually different but the non-suffixed version was the canonical one already referenced in `docs/ICON_MIGRATION.md`, so the `-icon.svg` copies were deleted. Barrel regenerated via `yarn generate-icons`

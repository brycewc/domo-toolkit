...
/**
 * Get all pages that a card appear on (including regular pages, app studio pages, and report builder pages)
 * @param {number} cardId - The card ID
 * @returns {Promise<Object>} Object with pageIds array and objectTypes array (parallel arrays)
 * @throws {Error} If the fetch fails
 */
export async function getPagesForCard({ cardId, tabId = null }) {
  try {
    // Execute fetch in page context to use authenticated session
    const result = await executeInPage(
      async (cardId) => {
        ...
        // Add regular pages with PAGE pageType
        if (card?.adminAllPages) {
          allPages.push(
            ...card.adminAllPages.map((page) => ({
              pageType: 'PAGE',
              pageId: page.pageId,
              pageTitle: page.title,
              parentPageId: page?.parentPageId || null,
              parentPageTitle: page?.parentPageTitle || null
            }))
          );
        }

        // Add app studio pages with DATA_APP_VIEW pageType
        if (card?.adminAllAppPages) {
          allPages.push(
            ...card.adminAllAppPages.map((page) => ({
              pageType: 'DATA_APP_VIEW',
              pageId: page.appPageId,
              pageTitle: page.appPageTitle,
              appId: page.appId,
        ...
        // Add report builder pages with REPORT_BUILDER_VIEW pageType
        if (card?.adminAllReportPages) {
          allPages.push(
            ...card.adminAllReportPages.map((page) => ({
              pageType: 'REPORT_BUILDER_VIEW',
              pageId: page.reportPageId,
              pageTitle: page.reportPageTitle || page.title || 'Page',
              reportId: page.reportId,
              reportTitle: page.reportTitle || page.reportName || 'Report'
            }))
          );
        }

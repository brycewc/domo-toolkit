...
Copy ID — Copy the current object's ID to your clipboard instantly. Long-press for related IDs like stream ID (for datasets) or studio app ID (for app studio pages). Keyboard shortcut: Ctrl+Shift+1 (Cmd+Shift+1 on Mac).
...
Delete Current Object — Delete beast modes, appdb collections, workflows, and pages/app studio pages including their cards — with a confirmation dialog and child page safety checks.
...
Get Cards — See every card, form, and queue on a page, app studio page, worksheet page, report builder page, or dataset. Items are grouped by type when a page contains multiple kinds. Open them all in new tabs with one click.
Get Pages — Discover where things are used:
For pages: View all child and grandchild pages in a hierarchical tree.
...
For any dataset: See all dependent datasets (views and fusions built on top of it) via the lineage API.
...
Pages, cards, datasets, dataflows, app studio apps/pages, worksheets, workspaces, report builder, users, groups, alerts, workflows, code engine packages, pro-code apps, beast modes, variables, access tokens, appdb collections, approvals, approval templates, drill paths, jupyter workspaces, filesets, files, forms, governance toolkit jobs, and dozens more.

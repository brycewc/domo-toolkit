...
- **Case object type names per `object-type-names.md`**: a bullet starts with a capital, so the type leading it is capitalized ("DataSets now support Delete."), but the same word mid-bullet is not ("Deleting a dataflow now lists the Beast Modes on its output datasets."). A button or view name quoted in a bullet keeps its own Title Case ("a Create DataSet button").
